...
app.get('/api/roles', async (request, response) => {
  ...
  const roles = await prisma.role.findMany({
    where: {
      ...(typeof request.query.company === 'string' ? { company: request.query.company } : {}),
      ...(field ? { field } : {}),
      ...(search ? { role: { contains: search } } : {})
    },
...
app.listen(port, () => console.log(`Skill Pulse API listening on http://localhost:${port}`));
