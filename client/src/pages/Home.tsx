...
export default function Home() {
  ...
  const selectedRole = roles.find((item) => String(item.id) === selectedRoleId);
  const { data: relatedPostings = [], isLoading: relatedPostingsLoading } = useQuery<Role[]>({
    queryKey: ['related-postings', selectedRole?.role],
    queryFn: async () => {
      if (!selectedRole) return [];
      const summaries = await api.roles(`search=${encodeURIComponent(selectedRole.role)}`);
      const matching = summaries.filter((item) => item.role === selectedRole.role);
      return Promise.all(matching.map((item) => api.role(String(item.id))));
    },
  ...
  return <main>
    <section className="bg-ink text-white"><div className="container-shell grid gap-12 py-20 lg:grid-cols-[1.15fr_0.85fr] lg:items-center lg:py-28">
      ...
      <div className="rounded-2xl bg-white p-3 text-ink shadow-2xl"><div className="p-5"><p className="text-sm font-bold">Find a role</p><p className="mt-1 text-sm text-slate-500">Choose a path and see its five core skills.</p><div className="mt-6 grid gap-3">
        <select aria-label="Company" value={company} onChange={(event) => { setCompany(event.target.value); setSelectedRoleId(''); }} className="w-full rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm outline-none focus:border-pulse"><option value="">Select company</option>{companies.map((item) => <option key={item}>{item}</option>)}</select>
        ...
        <select aria-label="Role" value={selectedRoleId} onChange={(event) => setSelectedRoleId(event.target.value)} disabled={!company} className="w-full rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm outline-none focus:border-pulse"><option value="">Select role</option>{roles.map((item) => <option key={item.id} value={item.id}>{item.role}</option>)}</select>
        <button disabled={!selectedRoleId} onClick={() => navigate(`/roles/${selectedRoleId}`)} className="flex items-center justify-center gap-2 rounded-lg bg-pulse px-4 py-3 text-sm font-bold text-white hover:bg-emerald-500 disabled:cursor-not-allowed disabled:opacity-50">View role skills <ArrowRight size={16} /></button>
      </div></div></div>
    </div></section>
    {selectedRole && <section className="container-shell py-16"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="eyebrow">Selected role postings</p><h2 className="mt-3 text-3xl font-extrabold tracking-tight">{selectedRole.role}</h2><p className="mt-3 text-slate-500">All matching {selectedRole.field} postings and their required skills.</p></div><Link to={`/roles/${selectedRole.id}`} className="inline-flex items-center gap-2 text-sm font-bold text-pulse">Start roadmap <ArrowRight size={16} /></Link></div>{relatedPostingsLoading ? <p className="mt-8 text-slate-500">Loading related postings...</p> : <div className="mt-8 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{relatedPostings.map((posting) => <Link to={`/roles/${posting.id}`} key={posting.id} className="card flex flex-col p-6 transition hover:-translate-y-1 hover:border-pulse"><div className="flex items-center justify-between gap-3"><span className="pill border-pulse text-pulse">{posting.field}</span><span className="text-sm text-slate-500">{posting.company}</span></div><h3 className="mt-5 text-xl font-extrabold">{posting.role}</h3><div className="mt-5 flex flex-wrap gap-2">{posting.skills.map((skill) => <span className="pill bg-mist text-xs" key={skill}>{skill}</span>)}</div><span className="mt-6 text-sm text-slate-500">Source: {posting.source}</span></Link>)}</div>}</section>}
