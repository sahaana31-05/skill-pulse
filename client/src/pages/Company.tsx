...
export default function Company() {
  ...
  return <main className="container-shell py-16">
    ...
    <p className="mt-4 max-w-2xl text-lg leading-8 text-slate-500">Explore every role from both fields in the dataset, including the five skills attached to each role.</p>
    <section className="mt-10 rounded-2xl bg-ink p-6 text-white sm:p-8"><p className="eyebrow">Learning roadmap</p><h2 className="mt-2 text-2xl font-extrabold">From field choice to course completion</h2><div className="mt-6 grid gap-4 md:grid-cols-4">{['Choose IT or Non-IT', 'Read the role content', 'Complete MCQs and check efficiency', 'Solve coding practicals and finish the mock'].map((step, index) => <div className="border-t border-white/20 pt-4" key={step}><span className="text-sm font-bold text-pulse">0{index + 1}</span><p className="mt-3 text-sm leading-6 text-slate-200">{step}</p></div>)}</div></section>
    {isLoading || detailsLoading ? <p className="mt-10 text-slate-500">Loading roles and Excel skills...</p> : <div className="mt-10 space-y-12">{fields.map((field) => {
