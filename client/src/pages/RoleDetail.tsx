...
export default function RoleDetail() {
  ...
  const submitSession = () => {
    ...
    if (!passed) return;
    setSessionMarks({ ...sessionMarks, [currentSession.number]: 5 });
    resetInputs();
    setSessionIndex(sessionIndex + 1);
  };
  ...
  return <main className="container-shell py-16">
    ...
    {currentSession && !courseComplete && !needsRetake && !showCheckpoint && <section className="mt-10 rounded-2xl border border-slate-200 bg-mist p-6 sm:p-8">
      <div className="flex flex-col justify-between gap-4 sm:flex-row"><div><p className="eyebrow">Module {moduleIndex + 1} of {courseSkills.length}</p><h2 className="mt-2 text-2xl font-extrabold">{currentSkill}</h2><p className="mt-2 text-base text-slate-500">Session {currentSession.number} of {sessions.length}: {currentSession.title}</p><p className="mt-1 text-sm text-slate-400">Topic: {currentSession.topic}</p></div><span className={`pill text-base ${currentSession.kind === 'Lecture' ? 'border-blue-300 text-blue-700' : currentSession.kind === 'Practical' ? 'border-amber-300 text-amber-700' : 'border-rose-300 text-rose-700'}`}>{currentSession.kind}</span></div>
      <div className="mt-8"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-sm font-bold uppercase tracking-wider text-slate-400">Course map</p><p className="mt-2 text-base text-slate-600">Open any session. A practical follows every three lectures: after lectures 3, 6, and 9.</p></div><span className="text-sm font-bold text-pulse">{Object.keys(sessionMarks).length} completed</span></div><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{sessions.map((session, index) => <button type="button" onClick={() => { setSessionIndex(index); setShowCheckpoint(false); resetInputs(); }} className={`min-h-20 rounded-xl border-2 px-4 py-3 text-left text-sm transition ${sessionIndex === index ? 'border-pulse bg-white text-pulse shadow-sm' : 'border-slate-200 bg-white text-slate-700 hover:border-emerald-300'}`} key={session.number}><span className="flex items-center justify-between gap-2"><span className="font-extrabold">{session.number}. {session.kind}</span>{sessionMarks[session.number] !== undefined && <span className="rounded-full bg-emerald-100 px-2 py-1 text-xs font-bold text-emerald-700">Mark {sessionMarks[session.number]}/5</span>}</span><span className="mt-2 block">{session.title}</span></button>)}</div></div>
