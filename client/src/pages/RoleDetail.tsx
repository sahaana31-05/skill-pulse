...
export default function RoleDetail() {
  ...
  const startCourse = async () => {
    ...
    setSessionIndex(0); resetInputs(); setModuleScores([]); setNeedsRetake(false); setCheckpointScore(null); setCheckpointMark(null); setCheckpointPassed(false); setShowCheckpoint(false); setCourseComplete(orderedSkills.length === 0);
  };
  ...
  const submitSession = () => {
    ...
    if (currentSession.kind === 'Lecture MCQ' || currentSession.kind === 'Final Mock') {
      ...
      setModuleScores([...moduleScores, score]); setCheckpointMark(mark); setCheckpointScore(score); setCheckpointPassed(mark > 3); setShowCheckpoint(true); resetInputs();
    ...
    if (currentSession.kind === 'Lecture') {
      const mark = currentSession.questions.filter((question, index) => answers[index] === question.answer).length;
      const score = getEfficiency(mark, currentSession.questions.length);
      setCheckpointMark(mark); setCheckpointScore(score); setCheckpointPassed(mark > 3); setShowCheckpoint(true); resetInputs();
      return;
    }
  ...
  const restartModule = () => { setNeedsRetake(false); setShowCheckpoint(false); setCheckpointScore(null); setCheckpointMark(null); setCheckpointPassed(false); setModuleScores(moduleScores.slice(0, -1)); setSessionIndex(0); resetInputs(); };
  const restartCourse = () => { setCourseComplete(false); setNeedsRetake(false); setShowCheckpoint(false); setCheckpointScore(null); setCheckpointMark(null); setCheckpointPassed(false); setModuleIndex(0); setSessionIndex(0); setModuleScores([]); resetInputs(); if (courseSkills.length) setSessions(createCourseSessions(courseSkills[0])); };
  const overallEfficiency = getEfficiency(moduleScores.reduce((sum, score) => sum + score, 0), moduleScores.length * 100);
  ...
  return <main className="container-shell py-16">
    ...
    {currentSession && !courseComplete && !needsRetake && !showCheckpoint && <section className="mt-10 rounded-2xl border border-slate-200 bg-mist p-6 sm:p-8">
      <div className="flex flex-col justify-between gap-4 sm:flex-row"><div><p className="eyebrow">Module {moduleIndex + 1} of {courseSkills.length}</p><h2 className="mt-2 text-2xl font-extrabold">{currentSkill}</h2><p className="mt-2 text-sm text-slate-500">Session {currentSession.number} of 17: {currentSession.title}</p><p className="mt-1 text-xs text-slate-400">Topic: {currentSession.topic}</p></div><span className={`pill ${currentSession.kind === 'Lecture' ? 'border-blue-300 text-blue-700' : currentSession.kind === 'Practical' ? 'border-amber-300 text-amber-700' : 'border-rose-300 text-rose-700'}`}>{currentSession.kind}</span></div>
      <div className="mt-6"><p className="text-xs font-bold uppercase tracking-wider text-slate-400">Precise session map</p><p className="mt-2 text-xs text-slate-500">Every session is open for review. Coding practicals follow lecture 3, lecture 6, lecture 9, the lecture MCQ, and the final lecture block. Completing a lecture requires at least 4 correct answers out of 5.</p><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{sessions.map((session, index) => <button type="button" onClick={() => { setSessionIndex(index); setShowCheckpoint(false); resetInputs(); }} className={`rounded-lg border px-3 py-2 text-left text-xs ${sessionIndex === index ? 'border-pulse bg-white text-pulse' : 'border-slate-200 bg-white text-slate-600'}`} key={session.number}><span className="font-bold">{session.number}. {session.kind}</span><span className="block mt-1">{session.title}</span></button>)}</div></div>
      <div className="mt-7 grid gap-5 lg:grid-cols-[0.9fr_1.1fr]"><div className="rounded-xl bg-white p-5"><p className="text-xs font-bold uppercase tracking-wider text-slate-400">Detailed content</p><h3 className="mt-2 font-extrabold">{currentSession.objective}</h3><div className="mt-5 space-y-4 text-sm leading-7 text-slate-600">{currentSession.content.split('\n\n').map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div></div><div className="rounded-xl bg-white p-5">{currentSession.kind === 'Practical' ? <><p className="text-xs font-bold uppercase tracking-wider text-slate-400">Coding practical</p><h3 className="mt-2 font-extrabold">{currentSession.codeScenario}</h3><textarea value={code} onChange={(event) => setCode(event.target.value)} placeholder="Write your solution here..." className="mt-5 min-h-48 w-full rounded-lg border border-slate-200 p-4 font-mono text-sm outline-none focus:border-pulse" /><button disabled={!code.trim()} onClick={submitSession} className="mt-4 rounded-lg bg-ink px-4 py-3 text-sm font-bold text-white disabled:opacity-40">Submit code</button></> : <><p className="text-xs font-bold uppercase tracking-wider text-slate-400">{currentSession.kind === 'Lecture MCQ' || currentSession.kind === 'Final Mock' ? 'MCQ assessment' : 'Lecture check'}</p>{currentSession.questions.map((question, questionIndex) => <div className="mt-5" key={question.prompt}><h3 className="font-extrabold">{question.prompt}</h3><div className="mt-3 grid gap-2">{question.choices.map((choice) => <button key={choice} onClick={() => setAnswers({ ...answers, [questionIndex]: choice })} className={`rounded-lg border px-4 py-3 text-left text-sm ${answers[questionIndex] === choice ? 'border-pulse bg-emerald-50' : 'border-slate-200'}`}>{choice}</button>)}</div></div>)}<button disabled={currentSession.questions.some((_question, index) => !answers[index])} onClick={submitSession} className="mt-5 rounded-lg bg-ink px-4 py-3 text-sm font-bold text-white disabled:opacity-40">{currentSession.kind === 'Lecture MCQ' ? 'Finish MCQ and show mark' : currentSession.kind === 'Final Mock' ? 'Finish final mock' : 'Submit lecture check'}</button></>}</div></div>
    </section>}
    {showCheckpoint && checkpointScore !== null && <section className="mt-10 rounded-2xl border-2 border-pulse bg-emerald-50 p-8"><p className="eyebrow">Session result</p><h2 className="mt-2 text-3xl font-extrabold">Mark: {checkpointMark} / 5</h2><p className="mt-2 text-xl font-bold text-pulse">Efficiency: {checkpointScore}%</p><p className="mt-4 text-slate-600">{needsRetake ? `You need at least ${PROFICIENCY_THRESHOLD}% to continue.` : currentSession?.kind === 'Lecture' ? 'Lecture passed. You can move to the next session.' : currentSession?.kind === 'Lecture MCQ' ? 'Lecture MCQ checkpoint passed. Coding practicals are ready.' : 'Final assessment passed.'}</p>{needsRetake ? <button onClick={restartModule} className="mt-6 inline-flex items-center gap-2 rounded-lg bg-ink px-4 py-3 text-sm font-bold text-white"><RotateCcw size={16} /> Retake course content</button> : checkpointPassed ? <button onClick={advanceAfterCheckpoint} className="mt-6 rounded-lg bg-pulse px-4 py-3 text-sm font-bold text-white">Continue to next session</button> : <button onClick={() => { setShowCheckpoint(false); setCheckpointScore(null); setCheckpointMark(null); setCheckpointPassed(false); resetInputs(); }} className="mt-6 rounded-lg bg-ink px-4 py-3 text-sm font-bold text-white">Retake this lecture</button>}</section>}
